...
public class Benchmark_Timer<T> implements Benchmark<T> {
    ...
    public static void main(String[] args){
        ...
        for (int i = startOfN; i < endOfN; i += i) {
            int doubledLength=i;
            Supplier<Integer[]> supplier = () -> {
                Integer[] a = new Integer[doubledLength];
                for (int j = 0; j < doubledLength; j++)
                    a[j] = StdRandom.uniform(-endOfN, endOfN);
                return a;
            };
            System.out.println("N= " + doubledLength +", Mean Time : " + benchmarkTimer.runFromSupplier(supplier, 100));
        }
        ...
        for (int i = startOfN; i < endOfN; i += i) {
            ...
            Supplier<Integer[]> supplier = () -> {
                ...
                for (int j = 0; j < doubledLength/2; j++)
                    a[j] = j;
                for (int j = doubledLength/2; j < doubledLength; j++)
                    a[j] = StdRandom.uniform(-endOfN, endOfN);
